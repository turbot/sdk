...
class Turbot {
  ...
  notify(controlId, icon, message, data) {
    ...
    switch (this.opts.type) {
      case "action": {
        ...
        // override
        if (controlId) {
          // this.meta.parentControlId is the parentControlId for all items generated by this action
          // do not override this value.
          command.payload.meta.parentControlId = controlId;
        }
  ...
  // Rules
  //
  // Deprecated pelase use prevention instead.
  get rule() {
    var self = this;
    return {
  ...
  get prevention() {
    ...
    return {
      ...
      update: function (preventionId, data, meta) {
        if (!preventionId && !data && !meta) {
          throw new errors.badRequest("Prevention ID and Data are mandatory");
        }
      ...
      delete: function (preventionIds) {
        const command = {
          type: "prevention_delete",
          meta: {
            preventionIds,
          },
        };

        if (!Array.isArray(preventionIds)) {
          preventionIds = [preventionIds];
        }

        let msg = `Delete preventions ${preventionIds.join(", ")}`;
        self.log.info(msg);
        self._command(command);
        return self;
      },
